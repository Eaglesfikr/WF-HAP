...
# 从增强数据中隔行取样
x_time_aug = x_time_aug_full

# --- 加载频域数据 ---
freq_data_orig = np.load('./datasets/awf1_freq.npz')
freq_data_aug = np.load('./datasets/awf1_freq_tfcaug.npz', mmap_mode='r')
    ...
    def train(self, train_loader):
        ...
        for epoch_counter in range(self.num_epoches + 1):
            ...
            if epoch_counter % 20 == 0 and epoch_counter > 0:
                ...
                torch.save(self.model.state_dict(), f'./checkpoints/WFTFC/WFTFC_all_with_tfctime__v727_epoch_{epoch_counter}.pth.tar')
