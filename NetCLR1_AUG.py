...
data = np.load('./datasets/awf2.npz') # AWF-PT-sup
# x_train = data['feature'] #awf1
x_train = data['data']      #awf2
# y_train = data['label']
y_train = data['labels']     #awf2
...
for i in tqdm.tqdm(range(n)):
    ...
    x_train_aug[2 * i] = augmentor.augment(trace)
    x_train_aug[2 * i + 1] = augmentor.augment(trace)
...
np.savez_compressed(
    "datasets/awf2_aug2x.npz",
    x_train=x_train_aug,
    y_train=y_train_aug
)

print("Saved to datasets/AWF/awf2_aug2x.npz")
...
print("Memory:",
      ...
      "MB")
# 磁盘文件大小
# disk_size = os.path.getsize(save_path)
# print(f"Disk:   {disk_size / 1024**2:.2f} MB ")
