...
time_data_aug = np.load('./datasets/awf1_time_tfcaug.npz')

x_train_orig = time_data_orig['feature'] # 原始频域特征
x_train_aug = time_data_aug['x']   # 增强后的频域特征
y_train = time_data_orig['label']      # 标签 (假设两个文件的标签顺序一致)

print(f"Original time data shape: {x_train_orig.shape}")
print(f"Augmented time data shape: {x_train_aug.shape}")
    ...
    def train(self, train_loader):
        ...
        for epoch_counter in range(self.num_epoches+1):
            ...
            if epoch_counter % 20 == 0 and epoch_counter > 0:
                ...
                torch.save(self.model.state_dict(), f'./checkpoints/WFTFC/WFTFC_time_epoch_{epoch_counter}.pth.tar')
