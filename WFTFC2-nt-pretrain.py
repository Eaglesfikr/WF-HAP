...
# ==========================================
# 加载你生成的两个频域文件
print("Loading termorl domain datasets...")
time_data_orig = np.load('./datasets/awf1.npz')
time_data_aug = np.load('./datasets/awf1_aug2x.npz')

x_train_orig = time_data_orig['feature'] # 原始频域特征
x_train_aug_full = time_data_aug['x_train']    # 增强后的频域特征 (包含2倍数据)
y_train = time_data_orig['label']        # 标签

print(f"Original time data shape: {x_train_orig.shape}")
print(f"Full Augmented time data shape: {x_train_aug_full.shape}")

# --- 关键修改：隔行取样 ---
# 假设增强文件的数据排列是 [aug1_sample0, aug2_sample0, aug1_sample1, aug2_sample1, ...]
# 我们使用切片 [::2] 来取所有偶数行 (索引为 0, 2, 4...)，即每个样本的第一个增强视图。
# 或者使用 [1::2] 来取所有奇数行 (索引为 1, 3, 5...)，即每个样本的第二个增强视图。
# 两者效果一样，任选其一即可。
x_train_aug = x_train_aug_full[::2] 

print(f"Selected Augmented time data shape: {x_train_aug.shape}")
    ...
    def train(self, train_loader):
        ...
        for epoch_counter in range(self.num_epoches+1):
            ...
            if epoch_counter % 20 == 0 and epoch_counter > 0:
                ...
                torch.save(self.model.state_dict(), f'./checkpoints/WFTFC/WFTFC_Ntime_epoch_{epoch_counter}.pth.tar')
...
if __name__ == "__main__":
    # 创建数据集和数据加载器
    train_dataset = TimeTrainData(x_train_orig, x_train_aug, y_train)
